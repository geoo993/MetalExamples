...
public class Camera {
    ...
    public var position: float3       // The position of the camera's centre of projection
    private var rotation: float3

    //view and projection matrix
    public var perspectiveProjectionMatrix: matrix_float4x4 // Perspective projection matrix
    public var orthographicProjectionMatrix: matrix_float4x4 // Orthographic projection matrix
    public var viewMatrix: matrix_float4x4

    public var view: float3 // The camera's viewpoint (point where the camera is looking)

    public var strafe: float3        // The camera's strafe vector

    public var front: float3               // The camera's forward vector
    public var back: float3                // The camera's backward vector
    public var left: float3               // The camera's left vector
    public var right: float3               // The camera's right vector
    public var up: float3                  // The camera's up vector
    public var down: float3                // The camera's down vector

    public var worldUp: float3            // The worlds up vector, the original position of the world

    // Eular Angles
    public var fieldOfView: Float           // The view from the camera
    ...
    // Camera options
    public var speed: Float         // How fast the camera moves
    public var sensitivity: Float      // How sensitive rotation is

    // Screen
    public var screenSize: CGSize           // size of the screen window
    ...
    public func updateCameraVectors()
    {
    ...
    public func set(position: float3, viewpoint: float3, up: float3) {
    ...
    public func rotateViewPoint(angle: Float, axis: float3) {
    ...
    public func rotateAroundPoint(distance: Float, viewpoint: float3, angle: Float, y: Float) {
    ...
    public func positionInFrontOfCamera(distance: Float) -> float3 {
        return position + front * distance
    }

    // Strafe the camera (side to side motion) (Left - Right Motion)
    public func strafe(direction: Float)
    {
    ...
    public func advance(direction: Float)
    {
    ...
    public func setRotation(angle: Float, displacement: Float, enabled: Bool) {
        if (enabled) {
    ...
    public func setTranslation(deltaTime: Float, angle: Float, displacement: Float)
    {
    ...
    public func updateRotation(angle: Float, displacement: Float, enabled: Bool = true) {
    ...
    public func updateMovement(deltaTime: Float, angle: Float, displacement: Float)
    {
    ...
    public func setPerspectiveProjectionMatrix(fieldOfView: Float, aspectRatio: Float, nearClippingPlane: Float, farClippingPlane: Float){
    ...
    public func setPerspectiveProjectionMatrix(screenSize: CGSize){
    ...
    public func setOrthographicProjectionMatrix(width: Float, height: Float, zNear: Float, zFar: Float){
        orthographicProjectionMatrix = ortho(left: 0, right: width, bottom: 0, top: height, zNear: zNear, zFar: zFar)
    }

    public func setOrthographicProjectionMatrix(value: Float, zNear: Float, zFar: Float)
    {
    ...
    public func computeNormalMatrix(modelMatrix: matrix_float4x4) -> matrix_float3x3
    {
