...
//
//  Created by GEORGE QUENTIN on 17/08/2018.
...
class MetalView : MTKView
{
    ...
    @IBOutlet weak var lightingLabel : UITextField!
    @IBOutlet weak var cubeShadowLabel : UITextField!
    @IBOutlet weak var multithreadUpdateLabel : UITextField!
    @IBOutlet weak var mtLabel: UITextField!
    @IBOutlet weak var frameEncodingTimeField: UITextField!
    @IBOutlet weak var drawCountField: UITextField!
    ...
    var camera  = WWDCCamera()
    var shadowCameras : Array<WWDCCamera> = [WWDCCamera]()
    ...
    override func awakeFromNib() {
        ...
        drawCountField?.text = "\(objectsToRender) draws"
        if multithreadedUpdate {
            multithreadUpdateLabel?.text = "Multithreaded Update"
        }
        else {
            multithreadUpdateLabel?.text = "Single Threaded Update"
        }

        if !multithreadedRender {
            mtLabel?.text = "Single Threaded Encode"
        }
        else {
            mtLabel?.text = "Multithreaded Encode"
        }
        ...
        for _ in 1...MAX_FRAMES_IN_FLIGHT {
            let buf : MTLBuffer = device!.makeBuffer(length: CONSTANT_BUFFER_SIZE, options: MTLResourceOptions.storageModeShared)!
        ...
        slider.addTarget(self, action: #selector(onSliderChanged(slider:event:)), for: .valueChanged)
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        */
        ...
        if frameCounter % 60 == 0 {
            frameEncodingTimeField?.text = String.localizedStringWithFormat("%.3f ms", mseconds)
        }
    ...
    @objc func onSliderChanged(slider: UISlider, event: UIEvent) {
        objectsToRender = Int(slider.value)
        drawCountField?.text = "\(objectsToRender) draws"
    }
