...
class LightingScene: Scene {
    ...
    override init(device: MTLDevice, camera: Camera) {
        ...
        torusKnot.position = float3(0, 0, 10)
        torusKnot.scale = float3(0.8)
        torusKnot.specularIntensity = 1.0
        torusKnot.shininess = 8.0

        light.position = float3(2, 20, -6)
        ...
        camera.set(position: float3(0,0,-1), viewpoint: torusKnot.position, up: float3(0,1,0))
    ...
    override func update(deltaTime: Float) {
        ...
        //camera.rotateAroundPoint(distance: 35, viewpoint: torusKnot.position, angle: cameraRotation, y: 0)
