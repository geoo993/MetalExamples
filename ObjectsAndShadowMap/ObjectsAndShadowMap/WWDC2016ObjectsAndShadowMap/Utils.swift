...
func createPlane(_ device : MTLDevice) -> (MTLBuffer, Int)
{
    ...
    let geoBuffer = device.makeBuffer(length: length, options: MTLResourceOptions.storageModeShared)!

    let geoPtr = geoBuffer.contents().bindMemory(to: CFloat.self, capacity: length)
    
	geoPtr.assign(from: &verts, count: verts.count)
    //geoBuffer. .__didModifyRange(NSMakeRange(0, verts.count*MemoryLayout<Float>.size))
